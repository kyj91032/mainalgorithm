# BFS: 가중치가 1인 그래프에서 목적지가 넓게 있을 때, 너비 우선으로 탐색해나가는 것. ex) 최단 거리 (깊게 있다고 판단할 수도 있음)

# BFS의 기본적인 동작 과정과 코드
'''
구체적인 탐색 과정: 인접 노드들을 우선으로 탐색하면서 전체를 점령함 (queue).

1. 탐색 시작 노드를 큐에 삽입하고 탐색 처리를 한다.

2. 큐에서 노드를 꺼낸다.
3. 해당 노드의 인접 노드 중에서 탐색하지 않은 노드를 모두 큐에 삽입하고 탐색 처리를 한다.
'''
