# 다이나믹(!= 동적, 런타임) 프로그래밍의 사용 조건 - 큰 문제와 작은 문제가 (as recurrence relation) 영향을 주며 존재하고, 똑같은 작은 문제를 반복적으로 해결함으로써 그 큰 문제가 해결될 때.
# (=> 영향이 존재(as recurrence relation) 하며 기억 공간이 필요 -> 다이나믹 프로그래밍 != 분할정복)
