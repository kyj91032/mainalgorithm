# DFS: 가중치가 1인 그래프에서 목적지가 깊게 있을 때, 깊이 우선으로 모든 조합을 탐색하는 것. ex) 백트래킹

# DFS의 기본적인 동작 과정과 코드
'''
구체적인 동작 과정: 인접 노드를 시작으로 깊게 기억하면서 전체를 점령함 (stack).

1. 탐색 시작 노드를 스택에 삽입하고 탐색 처리를 한다.

2. 스택의 최상단 노드가 탐색하지 않은 노드와 인접해 있다면 그 노드를 스택에 넣고 탐색 처리를 한다.
3. 탐색하지 않은 인접 노드가 없으면 스택에서 노드를 꺼낸다.
4. 2, 3을 스택이 빌 때까지(=재귀함수의 호출이 종료될 때 까지) 반복
